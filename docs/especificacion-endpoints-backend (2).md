...
> **Disclaimer — códigos de estado de Homecenter (`ESTADO_OC`/`ESTADO_SKU`).** La respuesta cruda de `GetOrdenesDeCompra` trae estos campos con un prefijo numérico (ej. `"1-PENDIENTE"`), no como el enum limpio que expone la API propia. De los prefijos reales solo `1-PENDIENTE` está confirmado contra una respuesta real; el resto (`PROCESANDO`, `DESPACHADA`, `CON_ERROR` a nivel de orden; `DESPACHADA`, `PARCIAL`, `CANCELADA`, `SUPERA_SOLICITADO` a nivel de línea) son **placeholders sin confirmar** — deben validarse con Homecenter antes de implementar la traducción real. Igual que con las fechas: normalizar el código crudo al enum limpio (`PENDIENTE`, `DESPACHADA`, etc.) es responsabilidad exclusiva del backend — el frontend nunca recibe ni interpreta el prefijo numérico, solo el valor ya traducido. `hc_integracion_log` es la única excepción visible, por la misma razón que con las fechas: guarda el payload crudo sin procesar.
